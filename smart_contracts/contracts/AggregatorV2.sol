...
contract Aggregator {
    ...
    function newEnergyRequirement(
        uint32 start,
        uint32 end,
        uint16 energyAmount
    ) external {
        EnergyRequirement memory r = EnergyRequirement(
            energyRequirementSequence,
            start,
            end,
            energyAmount
        ...
        energyRequirements[energyRequirementSequence] = r;
        userToEnergyRequirementIdsMap[msg.sender].push(
            energyRequirementSequence
        );
        energyRequirementIdToUserMap[energyRequirementSequence] = msg.sender;
        energyRequirementIdsForNextSchedule.push(energyRequirementSequence);
    ...
    function getMyEnergyRequirements()
        external
        view
        returns (EnergyRequirement[] memory)
    {
        uint16[] storage myIds = userToEnergyRequirementIdsMap[msg.sender];
        EnergyRequirement[] memory myReqs = new EnergyRequirement[](
            myIds.length
        );
        for (uint16 i = 0; i < myIds.length; i++) {
            myReqs[i] = energyRequirements[myIds[i]];
    ...
    function getEnergyRequirementsForNextSchedule()
        external
        view
        returns (EnergyRequirement[] memory)
    {
        EnergyRequirement[] memory nextScheduleReqs = new EnergyRequirement[](
            energyRequirementIdsForNextSchedule.length
        );
        for (
            uint16 i = 0;
            i < energyRequirementIdsForNextSchedule.length;
            i++
        ) {
            nextScheduleReqs[i] = energyRequirements[
                energyRequirementIdsForNextSchedule[i]
            ];
        }
        return nextScheduleReqs;
    }
    ...
    function storeSchedule(Schedule[] memory newSchedules) public onlyOwner {
        // Empty old schedules
        for (uint256 i = 0; i < schedulesIndex.length; i++) {
            address u = schedulesIndex[i];
            delete schedules[u];
        }
        delete schedulesIndex;
    ...
    // Schedule Retrieval (Frontend)
    function retrieveMyCurrentSchedule()
        external
        view
        returns (Schedule memory schedule)
    {
        return schedules[msg.sender];
    }
}